...
export default class WaveSurfer extends util.Observer {
    ...
    updateRestrictOptions(new_opts) {
        ...
        const needs_reset = ((!this.drawer.hasRestrictedCanvases && new_opts.restrict) ||
                             (new_opts.relativeTime != old_opts.relativeTime));

        this.params.restrictOptions = {...old_opts, ...new_opts};

        if (needs_reset) this.drawParamsChanged();
    ...
    createDrawer() {
        ...
        // Click-to-seek
        this.drawer.on('click', (e, click_pos) => {
            const seek_pos = this.viewOffsetToWaveformOffset(click_pos);
            setTimeout(() => this.seekTo(seek_pos), 0);
        });
